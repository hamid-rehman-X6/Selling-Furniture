...
import { storageGallery } from "./galleryimages";
...
const StorageGallery = () => {
  ...
  const nextImage = () => {
    setCurrentImageIndex(
      (prevIndex) => (prevIndex + 1) % storageGallery.length
    );
  };
  ...
  const prevImage = () => {
    setCurrentImageIndex(
      (prevIndex) =>
        (prevIndex - 1 + storageGallery.length) % storageGallery.length
    );
  };
      ...
      <div className="bg-[#F3F3F3]">
        ...
        <div className="mt-2">
          ...
          <h1 className="text-4xl font-montserrat font-extralight px-4 text-gray-800">
            STORAGE DESK - PROJECT IMAGES
          </h1>
        </div>
        <div className="p-4">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-2">
            {storageGallery.map(({ id, img }, index) => (
              <div
                key={id}
      ...
      {isModelOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-80 flex justify-center items-center z-50">
          ...
          <div className="relative flex items-center">
            ...
            <img
              src={storageGallery[currentImageIndex].img}
              alt={`Storage Desk ${storageGallery[currentImageIndex].id}`}
