...
import { ConferenceImages } from "./images";
import { useNavigate } from "react-router-dom";

const ConferenceSlider = () => {
  const [currentSlide, setCurrentSlide] = useState(0);
  const totalSlides = ConferenceImages.length;
  ...
  const handleNavigate = () => {
    navigate("/product-category/desks/conference-desk");
  };
    ...
    <>
      ...
      >
        <div className="w-full max-w-[1200px] overflow-hidden">
          ...
          >
            {ConferenceImages.map((imageItem) => (
              <div
                key={imageItem.id}
          ...
          <div className="relative w-full flex justify-center gap-3">
            {ConferenceImages.slice(0, totalSlides - imagesToShow + 1).map(
              (_, index) => (
                <button
                  key={index}
