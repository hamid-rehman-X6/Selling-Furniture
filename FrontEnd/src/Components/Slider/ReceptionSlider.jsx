...
import { ReceptionImages } from "./images";
import { useNavigate } from "react-router-dom";

const ReceptionSlider = () => {
  const [currentSlide, setCurrentSlide] = useState(0);
  const totalSlides = ReceptionImages.length;
  ...
  const handleNavigate = () => {
    navigate("/product-category/desks/reception-desk");
  };
      ...
      >
        <div className="w-full max-w-[1200px] overflow-hidden">
          ...
          <div className="relative w-full flex justify-center gap-3">
            {ReceptionImages.slice(0, totalSlides - imagesToShow + 1).map(
              (_, index) => (
                <button
                  key={index}
